@defcomp nice_recycle begin
    ...
    historic_emissions_PP_1990_05    = Parameter(index=[time, regions])
    historic_emissions_BP_1830_05 = Parameter(index=[time, regions])
    historic_damages_PP_00 = Parameter(index=[time, regions])
    historic_damages_BP_1990_05 = Parameter(index=[time, regions])
    ...
    cum_emissions = Variable(index=[time, regions])
    cum_damages = Variable(index=[time, regions])
    ...
    function run_timestep(p, v, d, t)
        ...
        for r in d.regions
            ...
            #Adding going forward L&D calcs here
