@defcomp nice_recycle begin
    ...
    historic_emissions_PP_1990_05    = Parameter(index=[regions])
    historic_emissions_BP_1830_05 = Parameter(index=[regions])
    historic_damages_PP_00 = Parameter(index=[regions])
    historic_damages_BP_1990_05 = Parameter(index=[regions])
    ...
    cum_emissions[t,r] = Variable(index=[time, regions])
    v.cum_damages[t,r] = Variable(index=[time, regions])
    ...
    function run_timestep(p, v, d, t)
        ...
        for r in d.regions
            ...
            #Add L&D calcs here
             if is_first(t)
            # Calculate period 1 cumulative emissions and damages
                v.cum_emissions[t,r] = p.historic_emissions_PP_1990_05[r] .+ p.industrial_emissions[t,r]
                v.cum_damages[t,r] = p.historic_damages_PP_00[r] .+ p.DAMAGES[t,r] 
                
            else
                v.cum_emissions[t,r] = v.cum_emissions[t,r] + p.industrial_emissions[t,r]
                v.cum_damages[t,r] = v.cum_damages[t,r] .+ p.DAMAGES[t,r]
            end
            
            v.emissionsshare[t,r] = (v.cum_emissions[t,r])./(sum(v.cum_emissions[t,:])
            v.damagesshare[t,r] = v.cum_damages[t,r] ./ sum(v.cum_damages[t,:])
