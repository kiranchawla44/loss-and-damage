    ...
    nice_recycle_objective =
        ...
        if revenue_recycling == true
            ...
            function(opt_tax::Array{Float64,1})
                ...
                return m[:rice_equivalent_welfare_component, :global_welfare]
            end
    ...
    fair_recycle_objective =
        ...
        if revenue_recycling == true
            ...
            function(opt_tax::Array{Float64,1})
                ...
                # Return total welfare.
                return nice[:rice_equivalent_welfare_component, :global_welfare]
            end
...
function save_nice_recycle_results(m_policy::Model, m_bau::Model, opt_tax::Array{Float64,1}, opt_tax_global_algorithm::Array{Float64,1}, output_directory::String; revenue_recycling::Bool=true)
    ...
    save(joinpath(regional_path, "nice_net_output.csv"), DataFrame(m_policy[:nice_recycle, :Y], :auto))
    save(joinpath(regional_path, "consumption.csv"), DataFrame(m_policy[:nice_recycle, :C], :auto))
    ...
    save(joinpath(regional_path, "damage_cost_share.csv"), DataFrame(m_policy[:nice_recycle, :DAMFRAC], :auto))
    save(joinpath(regional_path, "abatement_cost_share.csv"), DataFrame(m_policy[:nice_recycle, :ABATEFRAC], :auto))
