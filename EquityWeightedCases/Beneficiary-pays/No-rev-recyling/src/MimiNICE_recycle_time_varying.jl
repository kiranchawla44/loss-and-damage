...
historic_emissions_PP_1990_05 = Matrix(DataFrame(load(joinpath(@__DIR__, "..", "data", "historic_emissions_PP_1990_05.csv"), skiplines_begin=7))[:, 2:end]) ./ 1e1
historic_emissions_BP_1830_05 = Matrix(DataFrame(load(joinpath(@__DIR__, "..", "data", "historic_emissions_BP_1830_05.csv"), skiplines_begin=7))[:, 2:end]) ./ 1e1
historic_damages_PP_00 = Matrix(DataFrame(load(joinpath(@__DIR__, "..", "data", "historic_damages_PP_00.csv"), skiplines_begin=7))[:, 2:end]) ./ 1e1
historic_damages_BP_1990_05 = Matrix(DataFrame(load(joinpath(@__DIR__, "..", "data", "historic_damages_BP_1990_05.csv"), skiplines_begin=7))[:, 2:end]) ./ 1e1
...
function create_nice_recycle(;slope_type::Symbol=:central, percentile::Float64=0.90)
    ...
    set_param!(nice_rr, :nice_recycle, :historic_emissions_PP_1990_05,   historic_emissions_PP_1990_05)
    set_param!(nice_rr, :nice_recycle, :historic_emissions_BP_1830_05,   historic_emissions_BP_1830_05)
    set_param!(nice_rr, :nice_recycle, :historic_damages_PP_00,   historic_damages_PP_00)
    set_param!(nice_rr, :nice_recycle, :historic_damages_BP_1990_05,   historic_damages_BP_1990_05)
